  export async function POST(req: Request) {
    ...
    const { id, recipeTitle, imgURL, prepTime, ingredients, steps } = await req.json();
    console.log("EDIT API ROUTE");
    ...
    if (!id || !recipeTitle || !imgURL || !prepTime || !ingredients || !steps) {
    ...
    return new Response(JSON.stringify({ id, recipeTitle, imgURL, prepTime, ingredients, steps }), {
      status: 200,
      headers: {
